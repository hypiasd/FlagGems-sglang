...
four-row batch. v14 keeps those. It removes the v13 multi-head tail
specialization that coincided with the Hygon and international regressions,
and returns Kunlunxin's contiguous multi-head schedule to the v12 head-tile
path after its row-batch score fell from `0.31x` to `0.28x`. A new, narrow
optimization remains: when the input has exactly one head, every head-tile
wrapper launches a one-head tile, avoiding guaranteed masked lanes without
changing the multi-head schedule.
